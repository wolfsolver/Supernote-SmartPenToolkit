...
import { PluginCommAPI, PluginManager, PluginFileAPI, PointUtils, type Point } from 'sn-plugin-lib';
...
const Main = ({ onClose }: SettingProps) => {
    ...
    useEffect(() => {
        const processElementsOnStartup = async () => {
            log("Main", "Processing elements on startup");
            setIsProcessing(true);
            try {
                // 1. Get current file path and page number
                log("Main", "Locating current page...");
                ...
                log("Main", "Reading elements on page " + pageNum);
    ...
    async function detectShape(el: any) {
        log("Main", "Detecting shape for element " + el.uuid);
        try {
 ...
 */
    async function insertShapeGeometry(shape: any) {
        log("Main", "Inserting shape geometry for " + shape.type);
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.title}>[{config.name}] Shape Processing</Text>
